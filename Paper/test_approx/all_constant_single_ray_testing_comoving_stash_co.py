...
#Note: difference between this and the other testing file: different test molucule, thus different line widths (and far down the calculation, different amounts of diffusive behavior)
...
def create_model ():
    ...
    modelName = f'all_constant_single_ray_testing_comoving_stash_co'
...
def run_model (nosave=False):
    ...
    print("Icmb: ", tools.I_CMB(line_freq[0]))
    print("dfreqs for line 0:", line_freq[0]* inv_line_width[0, 0])
    print("in units of div c", 1/(line_freq[0]* inv_line_width[0, 0]))
    print("times max line width", 1/(line_freq[0]* inv_line_width[0, 0])*max_line_width)
...
#TODO: figure out a way to compare this stuff
    ...
    model.compute_Jeff_sparse()
    timer6.stop()
    J_coapp = np.array(model.lines.lineProducingSpecies[0].Jlin)
    ...
    print("mean abs rel diff feautrier comoving: ", np.mean(np.abs(J_2f[:,0]-J_co[:,0])/(J_2f[:,0])))
    ...
    if not nosave:
        ...
        plt.plot(x, J_2f[:,0], label='Feautrier', zorder=1)
        plt.plot(x, J_co[:,0], label='Comoving', zorder=1)
        plt.plot(x, J_coapp[:,0], label='Comoving approx', zorder=1)
        ...
        # plt.scatter(x, u_2f[0,:,0], s=0.5, label='2f', zorder=1)
        # plt.scatter(x, J_0s[:,0], s=0.5, label='0s', zorder=1)
        plt.plot(x, J_2f[:,0]/J_2f[:,0], label='Feautrier', zorder=1)
        plt.plot(x, J_co[:,0]/J_2f[:,0], label='Comoving', zorder=1)
        plt.plot(x, J_coapp[:,0]/J_2f[:,0], label='Comoving approx', zorder=1)
        plt.hlines(1.02,np.min(x), np.max(x))
        plt.hlines(1-0.02, np.min(x), np.max(x))
        # plt.plot(x, u_(x), c='lightgray', zorder=0)
        plt.legend()
        # plt.xscale('log')
        plt.xlabel('x [m]')
